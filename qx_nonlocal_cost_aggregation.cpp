...
int qx_nonlocal_cost_aggregation::matching_cost(int s,unsigned char***left,unsigned char***right)
{
	
	matching_cost_from_color_and_gradient(s,left,right);
	image_copy(m_cost_vol_backup,m_cost_vol,m_h,m_w,m_nr_plane);
	qx_stereo_flip_corr_vol(s,m_cost_vol_right,m_cost_vol,m_h,m_w,m_nr_plane);
	
	
    return(0);
}
int qx_nonlocal_cost_aggregation::disparity(int s,unsigned char**disparity,int use_nonlocal_post_processing)
{
	int radius=2;
	if (use_nonlocal_post_processing == 0) {
		image_copy(m_cost_vol, m_cost_vol_backup, m_h, m_w, m_nr_plane);
		m_tf.filter(m_cost_vol[0][0], m_cost_vol_temp[0][0], m_nr_plane);
		depth_best_cost(s, m_disparity, m_cost_vol,m_cost_min,m_h, m_w, m_nr_plane);
		if(s+m_nr_plane== max_disparity)
		ctmf(m_disparity[0], disparity[0], m_w, m_h, m_w, m_w, radius, 1, m_h*m_w);
		//image_copy(disparity,m_disparity,m_h,m_w);
	}
	else if (use_nonlocal_post_processing == 1)
	{
		
		image_copy(m_cost_vol_r, m_cost_vol_right, m_h, m_w, m_nr_plane);
		m_tf_right.filter(m_cost_vol_r[0][0], m_cost_vol_temp[0][0], m_nr_plane);
	   
	

		depth_best_cost(s,m_disparity_r, m_cost_vol_r, m_cost_min_right,m_h, m_w, m_nr_plane);
		
		if (s + m_nr_plane == max_disparity)
		ctmf(m_disparity_r[0], m_disparity_right[0], m_w, m_h, m_w, m_w, radius, 1, m_h*m_w);
		...
		for(int y=0;y<m_h;y++) for(int x=0;x<m_w;x++) if(!m_mask_occlusion[y][x])
		{
			for(int d=0;d<m_nr_plane;d++) 
				...
				m_cost_vol[y][x][d]=abs(disparity[y][x]-d-s);
		}
...
void qx_nonlocal_cost_aggregation::matching_cost_from_color_and_gradient(int s,unsigned char ***left,unsigned char ***right)
{
	image_zero(m_cost_vol,m_h,m_w,m_nr_plane);
	...
	for(int i=0;i<m_nr_plane;i++)
	{
		...
		{
			int j = s + i;
			image_copy((&m_image_shifted[y][j]),right[y],m_w-j,3);
			memcpy(&(m_gradient_shifted[y][j]),m_gradient_right[y],sizeof(float)*(m_w-j));
			for(int x=0;x<j;x++) 
			{
