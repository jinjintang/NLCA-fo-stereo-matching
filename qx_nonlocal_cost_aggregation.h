...
public:
	...
	int	m_h, m_w, m_nr_plane,max_disparity; double m_max_color_difference, m_max_gradient_difference, m_weight_on_color, m_weight_on_color_inv;
	unsigned char**m_disparity, **m_disparity_r, **m_disparity_right, **m_disparity_temp,**m_mask_occlusion, ***m_buf_u2;
	double m_table[256], m_sigma_range;
	unsigned char***m_left, ***m_right, ****m_buf_u3, ***m_image_shifted, ***m_image_temp;
	float***m_buf_f2, **m_cost_min, **m_cost_min_right, **m_cost_temp, **m_cost, **m_gradient_left, **m_gradient_right, **m_gradient_shifted;
	double****m_buf_d3, ***m_cost_vol, ***m_cost_vol_r, ***m_cost_vol_right, ***m_cost_vol_backup, ***m_cost_vol_temp;
    ...
    int matching_cost(int s,unsigned char***left,unsigned char***right);
    int disparity(int s,unsigned char**disparity,int use_nonlocal_post_processing);
private:
	void matching_cost_from_color_and_gradient(int s,unsigned char***left,unsigned char***right);
