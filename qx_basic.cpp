...
void qx_stereo_flip_corr_vol(int s,double***corr_vol_right,double***corr_vol,int h,int w,int nr_plane)
{	
	for(int y=0;y<h;y++)
	{
		for(int x=0;x<w-nr_plane-s;x++) for(int d=0;d<nr_plane;d++) corr_vol_right[y][x][d]=corr_vol[y][x+d+s][d];
		for(int x=w-nr_plane-s;x<w;x++) for(int d=0;d<nr_plane;d++)
		{
			if((x+d+s)<w) corr_vol_right[y][x][d]=corr_vol[y][x+d+s][d];
			else corr_vol_right[y][x][d] = corr_vol[y][x][d-1];
		}
	}
}
void depth_best_cost(int s,unsigned char**depth,double***evidence,float **minv,int h,int w,int nr_planes)
{
	for(int y=0;y<h;y++) for(int x=0;x<w;x++) {int d;
	if (s!=0)d = depth[y][x];
	vec_min_pos(s,d,evidence[y][x],minv[y][x],nr_planes); depth[y][x]=d;
	
	}
}
void vec_min_pos(int s,int &min_pos,double *in,float &minv,int len)
{
	
	if (!s) { min_pos = 0; 
	minv = in[0];
	}
	for (int i=0;i<len;i++) if(in[i]<minv)
	{
		minv=in[i];	
		min_pos= i+s;
